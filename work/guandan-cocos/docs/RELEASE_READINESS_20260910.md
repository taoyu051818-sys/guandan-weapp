...
首轮 GitHub 检查发现转蛋 smoke 的服务启动失败；旧脚本丢弃子进程日志，不能准确回溯底层启动原因。本轮将带副作用的三个模式测试显式注册到默认测试链，以独立进程顺序执行，转蛋使用临时端口、隔离环境，并保留有界启动日志和失败清理；规则断言不减少。不要用“本地通过”掩盖远程失败，应以候选提交重新运行的 CI 为准。
