...
function verifySourceConversions () {
  ...
  assert.match(tableHandInteractionController, /public playSelected[\s\S]*!snapshot\.playValidation\.canPlay[\s\S]*showToast\(this\.dependencies\.validationHint\(snapshot\.playValidation\)\)[\s\S]*ruleAuthority\.playSelected\(\)/, 'a visible play action must explain an invalid selection while retaining authoritative submission validation')
...
function verifyTurnInteractionLifecycle () {
  ...
  assert.equal(canSelectPlayingHand(state('p2'), 'p1', false), true, 'off-turn cards must accept local preselection')
