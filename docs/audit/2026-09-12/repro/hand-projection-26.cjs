// Audit-only, synthetic hands. No network, files generated by product, or product mutations.
'use strict'
const assert = require('node:assert/strict')
const fs = require('node:fs'), path = require('node:path')
const root = path.resolve(__dirname, '../../../..'), app = path.join(root, 'work/guandan-cocos')
...
function aceLowOrdering () {
  ...
  for (const suit of suits) for (const wild of [false,true]) {
    ...
    for(const profile of [profiles[0], {...profiles[0], straightFlushAsBomb:false}]) for(const route of ['auto','lock']) {
      ...
      if(profile.straightFlushAsBomb){
        assert.deepEqual(actual,[2,3,4,5,14])
  ...
  return {confirmedMismatches:mismatches.length,controls:controls.length,example:mismatches[0],
...
function publicCounter () {
  ...
  for(const level of core.MATCH_LEVELS)for(const suit of suits)for(const sequence of sequences){
    ...
    assert(inferred.includes(suit))
    const plainLevel=unseen.some(c=>c.isLevelCard&&!c.isRedJoker)
    if(plainLevel){
      assert.equal(info,null);falsePositives++
    ...
    }else{assert.equal(info.type,core.PlayType.StraightFlush);legalControls++}
  ...
  return {cases,falsePositives,legalControls,wildcardRepairs,duplicateChecks,example,
