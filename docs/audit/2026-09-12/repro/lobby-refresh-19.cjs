// Audit-only: real lobby/state/profile presenter with inert UI/network collaborators.
...
const modes=require(path.join(root,'shared-core/dist/lib/classicModes.js'))
...
async function main(){
  ...
  for(const route of ['settings','classic'])for(const order of [['dashboard','wallet'],['wallet','dashboard']]){
    ...
    assert.equal(h.rendered,before,'old request token prevents repaint after returning')
    assert.ok(h.labels.includes('账号同步中'));assert.ok(h.labels.includes('--'))
    h.timers.splice(0).forEach(cb=>cb());await flush()
    assert.ok(h.labels.includes('账号同步中'),'only scheduled invite activation does not refresh identity')
    h.domain.reflow()
    assert.ok(h.labels.includes('已同步用户'));assert.ok(h.labels.includes('5200'))
    cases.push({route,order,storedFresh:true,visibleStale:true,manualReflowRecovers:true});h.destroy()
  }
