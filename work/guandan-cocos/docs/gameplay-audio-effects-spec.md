...
当前已从 MIT 授权的 NiuMa Cocos 工程接入完整且语义明确的 Female 报牌矩阵：全部单张点数、全部普通对子、王对子通用报读，以及三张、顺子、三带二、三连对、同花顺、炸弹和王炸。2026-09-07 另补 Edge TTS 晓晓女声“钢板”，按 PlayType.Plate 单次播报；男声包退役，旧男声设置自动归一化。代码、资源、Cocos `.meta` 与 SHA-256 校验通过，尚未完成新包真机听感验收，因此仍标为“源码已配置，待复验”。普通出牌不再叠加电子落牌声，也不得把旧 `feiji.mp3` 误报为钢板。
...
| 钢板 | 2026-09-07 新增晓晓 TTS 女声，0.585 秒本地 WAV；不播放电子落牌声 | 飞牌、“钢板”标签、通用流光 | 语义映射/文件校验通过，真机听感待验 |
...
3. 真机逐项复验完整 Female 报牌矩阵、三种“不要”、倒计时、局开始/胜负声音及新增“钢板”TTS 女声，重点听句尾完整性和跨音色响度。
