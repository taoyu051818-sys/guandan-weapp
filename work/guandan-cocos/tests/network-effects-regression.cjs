...
assert.match(effectController, /decideActionEffectSync\(this\.lastActionCount, actions\.length\)/, 'effect playback must use the tested action policy')
assert.match(effectController, /sync !== 'duplicate'[\s\S]*this\.pendingLocalOrigins = \[\]/, 'duplicate pending renders must preserve captured card origins')
assert.match(lobbyController, /decideNetworkEffectSync\(this\.effectSyncCursor/, 'network packets must use the tested version policy')
assert.match(lobbyController, /pending\.requestType === 'rejoinRoom' \? 'reconnect' : 'initial-snapshot'/, 'entry snapshots must declare recovery intent')
assert.match(gameScene, /if \(packet\.effectSync\.mode === 'recovery'\) this\.prepareRecoveryVisualBaseline/, 'only recovery packets may reset presentation state')
const applyNetworkState = gameScene.slice(gameScene.indexOf('private applyNetworkState'), gameScene.indexOf('private applyNetworkRoundPrepared'))
