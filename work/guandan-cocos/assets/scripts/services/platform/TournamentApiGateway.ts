import type { TournamentGateway, TournamentStandings, TournamentState, TournamentSummary } from '../FrontPageGatewayContracts'
...
import { idempotencyKey, malformedResponse, nonNegativeInteger, requireArray, requireBoolean, requireRecord } from './validation'

export class TournamentApiGateway implements TournamentGateway {
  ...
  public async withdraw (tournamentId: string): Promise<TournamentState> {
    const id = this.safeTournamentId(tournamentId)
    const payload = await this.client.request<unknown>(`/api/v1/tournaments/${encodeURIComponent(id)}/withdraw`, 'POST', {}, {
      'Idempotency-Key': idempotencyKey('withdraw'),
    })
    return this.normalizedStateFor(payload, id)
  }
