...
const getMaxBomb = (
  ...
  let bestInfo: { type: PlayType; maxValue: number; length?: number } | null = null;
  for (const b of bombs) {
    const info = memoGetPlayInfo(b);
    if (!info) continue;
    if (!bestInfo || compareBombResolutions(info, bestInfo) > 0) {
      bestInfo = info;
      best = b;
    }
  }
...
const triplePairUsesSmallPair = (
  play: Card[],
  memoGetPlayInfo: (cards: Card[]) => { type: PlayType; maxValue: number; length?: number } | null
): boolean => {
...
const isSmallBomb = (
  ...
  return !!info && info.type === PlayType.Bomb && info.maxValue % 1000 <= 7;
};
