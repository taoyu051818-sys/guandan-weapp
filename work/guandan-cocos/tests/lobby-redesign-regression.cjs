...
assert.match(frontPage, /entryClassic:[^\n]+entry-classic\/texture/)
assert.match(frontPage, /entryFriend:[^\n]+entry-friend\/texture/)
assert.match(frontPage, /entryTournament:[^\n]+entry-tournament\/texture/)
assert.doesNotMatch(frontPage, /renderRankMonument|LobbyRankBase|LobbyFloatingGem|rankBase|rankGem/, 'the retired rank monument and floating gem must not return to the lobby runtime')
...
assert.match(frontPage, /ClassicEntryCard[\s\S]*FriendEntryCard[\s\S]*TournamentEntryCard/, 'the main lobby must expose exactly distinguishable classic, friend-room and tournament cards')
assert.match(frontPage, /score: 50, queueId: 'classic_50'/)
assert.match(frontPage, /name: '初级场', score: 50/)
assert.match(frontPage, /score: 300, queueId: 'classic_300'/)
assert.match(frontPage, /score: 2000, queueId: 'classic_2000'/)
assert.match(frontPage, /score: 10000, queueId: 'classic_10000'/)
const tierDefinition = frontPage.match(/const CLASSIC_ROOM_TIERS[\s\S]*?\n\]/)?.[0] ?? ''
assert.doesNotMatch(tierDefinition, /difficulty/, 'classic room tiers must never select AI difficulty')
assert.match(frontPage, /startClassicTier[\s\S]*?beginMatch\(tier\.queueId,[\s\S]*?'classic-rooms'\)/, 'classic room cards must enter distinct human matchmaking queues')
assert.match(frontPage, /to\(0\.34, \{ position: new Vec3\(targetX, cardY, 0\) \}/, 'classic room cards must enter from right to left')
assert.match(frontPage, /锦标赛[\s\S]*老乡赛[\s\S]*校友赛[\s\S]*自建赛[\s\S]*我的比赛/)
assert.match(frontPage, /经典过A\s+·\s+四人组队\s+·\s+服务器验牌/)
assert.match(frontPage, /renderShopShortcut[\s\S]*ShopChickArtwork[\s\S]*'商城'/)
assert.match(frontPage, /coin:[^\n]+ui\/lobby\/coin\/texture/)
assert.match(frontPage, /LobbyCoinIcon[\s\S]*LOBBY_ART\.coin/, 'the lobby points row must use the transparent coin asset')
const shopShortcut = frontPage.match(/private renderShopShortcut[\s\S]*?\n  }/)?.[0] ?? ''
...
assert.match(frontPage, /defaultAvatar:[^\n]+ui\/common\/default-avatar\/texture/, 'the lobby must load the supplied image instead of a text avatar placeholder')
const showMenuSection = frontPage.slice(frontPage.indexOf('public showMenu'), frontPage.indexOf('private renderMenuPage'))
assert.match(showMenuSection, /if \(this\.gateways\.configured\) \{[\s\S]*this\.playerDashboard = null[\s\S]*this\.playerDashboardLoadedAt = 0[\s\S]*this\.walletFresh = false[\s\S]*\}/, 'each online lobby entry must invalidate cached profile and wallet freshness before refreshing')
const profileSection = frontPage.slice(frontPage.indexOf('private renderLobbyPlayerProfile'), frontPage.indexOf('private resolveLocalAccountId'))
assert.ok(profileSection.length > 0, 'the horizontal lobby identity lane must exist')
assert.match(profileSection, /const hasRemoteDashboard = this\.playerDashboard !== null[\s\S]*const awaitingRemoteDashboard = this\.gateways\.configured && !hasRemoteDashboard/, 'online profile placeholders must be driven by remote dashboard availability')
assert.match(profileSection, /const displayName = awaitingRemoteDashboard \? '账号同步中' :/, 'an online lobby without profile data must not show a sample username')
assert.match(profileSection, /const accountId = [^\n]*remoteAccountId[^\n]*this\.gateways\.configured \? '同步中' : this\.localAccountId/, 'an online lobby without profile data must not expose the locally generated account id')
assert.match(profileSection, /ui\.outlinedLabel\(`ID \$\{accountId\}`/, 'the synchronized account placeholder must render as ID 同步中')
assert.match(profileSection, /const comprehensiveScore = awaitingRemoteDashboard[\s\S]*\? null[\s\S]*const pointsText = this\.gateways\.configured && !this\.walletFresh \? '积分 --' : `积分 \$\{Math\.max\(0, Math\.round\(this\.wallet\.points\)\)\}`[\s\S]*const comprehensiveText = `综合 \$\{comprehensiveScore \?\? '--'\}`/, 'online loading state must show 积分 -- and 综合 -- instead of local defaults')
...
assert.match(profileSection, /ui\.panel\('LobbyIdentityPill', identity\.x, y, identity\.width, 60, \{ fill: new Color\(4, 8, 8, 172\), lineWidth: 0, radius: 30 \}\)/, 'the identity field must use a half-height black translucent capsule')
...
assert.match(profileSection, /LobbyPlayerProfileHitArea[\s\S]*void this\.showPlayerCenter\(\)/, 'touching the lobby identity lane must open the unified player center')
assert.doesNotMatch(profileSection, /this\.host\.showNotice\(/, 'the lobby identity lane must not stop at an informational popup')
assert.match(frontPage, /LOCAL_ACCOUNT_ID_STORAGE_KEY = 'guandan-local-account-id-v1'/)
assert.match(frontPage, /\^\\d\{8\}\$[\s\S]*String\(10_000_000 \+ random % 90_000_000\)[\s\S]*localStorage\.setItem/, 'the local account id must be exactly eight digits and persist across launches')
assert.match(frontPage, /rawWinRateText = awaitingRemoteDashboard \? '--' : games > 0 \? rawWinRate\.toFixed\(1\) : '0'/, 'a synchronized player with no games must display 0%, while pending data remains unknown')
assert.match(frontPage, /adjustedWinRate = \(Math\.max\(0, wins\) \+ 25\) \/ \(safeGames \+ 50\)/)
assert.match(frontPage, /experience = 0\.3 \+ 0\.7 \* Math\.log1p\(safeGames\) \/ Math\.log\(101\)/)
assert.match(frontPage, /baseScore = 60_000 \* Math\.pow\(adjustedWinRate, 1\.8\) \* experience/)
assert.match(frontPage, /return Math\.max\(1_000, Math\.round\(baseScore \+ Math\.max\(0, legacyElo\) - 1_000\)\)/)
...
assert.match(frontPage, /beginMatch\('classic_50', '经典 · 初级场 · 底分50', 'menu'\)/, 'quick start must enter the lowest classic matchmaking queue')
assert.match(frontPage, /returnPage === 'menu'\) this\.showMenu\(\)/, 'quick-start cancellation and failures must return to the main lobby')
const matchErrorSection = frontPage.slice(frontPage.indexOf('private matchErrorDetail'), frontPage.lastIndexOf('\n}'))
...
assert.doesNotMatch(frontPage, /匹配入桌失败/)
assert.equal((frontPage.match(/比赛匹配失败/g) ?? []).length, 3, 'all matchmaking failures must use the requested competition wording')
...
const friendTableSection = frontPage.slice(frontPage.indexOf('private renderFriendTableLobby'), frontPage.indexOf('public hideAll'))
...
const friendSettingsSection = frontPage.slice(frontPage.indexOf('private showFriendRoomSettings'), frontPage.indexOf('public showOnlinePlay'))
assert.match(frontPage, /friendSettingsTab: 'rules' \| 'experience' = 'rules'/)
...
assert.doesNotMatch(frontPage, /切换 AI 难度|setDifficulty\s*\(/)
