...
| 选牌 | 无 | 牌上移 32px、金边、按压缩放 | 可加极轻点击声；连续拖选时需限频 |
| 提示 | 无 | 自动选中一组合法牌，显示重置与出牌按钮 | 可加短促提示声，不能和报牌语音混淆 |
...
图 5：手牌选中后上移并显示金色边缘。该画面用于验收“点按有反馈”和“按钮按可用动作动态居中”。

![提示自动选牌](assets/game-flow/08-hint-selection.png)

图 6：点击“提示”后自动选中可出的对子，同时出现“重置”和“出牌”。
...
- 授权 CDN 的 `pair_a_phrase.mp3` 仍只归档；对 A 改用语义明确的 NiuMa `2_14.mp3`。
