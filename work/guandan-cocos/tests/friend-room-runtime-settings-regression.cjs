...
assert.match(tableHud, /const hideScores = lobby\?\.roomSettings\?\.scoreVisibility === 'hidden' && snapshot\.phase === 'playing'/, 'scores hide only during play, preserving true settlement results')
assert.match(tableHud, /duplicateTableLabel\(lobby\?\.duplicate, hideScores, snapshot\.phase\) \?\? projectTableModeLabel\(snapshot\.state, humanId, hideScores/, 'duplicate and rotating modes share the same score-visibility projection')
