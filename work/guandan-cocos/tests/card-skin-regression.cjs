...
function verifySingleRendererBoundary () {
  ...
  assert.doesNotMatch(cardView, /\bRichText\b|CardText|CenterSuit|showCodeFallback|CardSkinRegistry|requestedSkin|setSkin\s*\(/, 'CardView must not contain a second text/code card renderer')
  assert.equal((cardView.match(/addComponent\(Label\)/g) || []).length, 1, 'the only text overlay is the level corner marker, not rank/suit fallback')
  ...
  assert.doesNotMatch(cardView, /CardLevelYellowFilter|levelFilter|new Color\(255, 190, 28, 54\)/, 'level cards must never tint the complete face yellow')
