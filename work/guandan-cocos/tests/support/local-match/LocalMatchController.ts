...
export class LocalMatchController {
  ...
  public static start (options: LocalMatchStartOptions): LocalMatchController {
    ...
    const level = options.matchFormat ? chooseMatchLevel(options.matchFormat, random) : options.level
    const game = createGame(level, options.dealerId, options.ruleProfile, random)
    return new LocalMatchController(createMatchState({
      ...
      teamLevels: options.matchFormat ? { teamA: level, teamB: level } : options.teamLevels,
  ...
  public prepareNextRound (): TransitionResult {
    ...
    const nextLevel = this.match.matchFormat?.kind === 'independent'
      ? chooseMatchLevel(this.match.matchFormat, this.random) : this.match.settlement.currentLevel
    return this.dispatch({
      type: 'PREPARE_NEXT_ROUND',
      nextLevel,
      dealtHands: dealCards(shuffleDeck(createDeck(nextLevel), this.random)),
    })
  }
