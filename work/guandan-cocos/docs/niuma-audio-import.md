# NiuMa 男女独立报牌、牌局流程与背景音乐接入

本次从 MIT 仓库 `niuma-wj/client-cocos` 的固定提交 `f9d037feaef5a80867fd97c8dd39b9a7486fbeca` 选择默认女声 48 个 MP3 + 1 个 OGG、可选男声 39 个 MP3 + 1 个 OGG，以及 1 条 MP3 背景音乐。运行时继续通过 `AudioProfiles`、`PlayVoiceProfiles`、`QuickChatPolicy` 和 `CocosAudioController` 使用语义键，不允许牌局或场景代码直接引用文件名。

## 已接入

- 单张：`2–10、J、Q、K、A、小王、大王` 全量女声。
- 对子：`2–10、J、Q、K、A` 全量女声；大小王对子使用通用“对子”，已有更具体的“对小王”仍优先。
- 男声包：与女声相同的单张、对子、三张、顺子、三带二、三连对、同花顺、炸弹、王炸和三种“不要”；在设置页显式切换，绝不男女随机或跨声线回退。
- 牌型：三张、顺子、三带二、三连对、同花顺、炸弹、王炸。
- 流程：三种“不要”轮换、倒计时 `0–5` 精确选择、本局开始、胜利、失败。
- 快捷语：Female/Male 均只接入上游 `phrase02`，并把当前按钮/服务端白名单统一为原声确切文案“你的牌打得太好啦”。
- 背景音乐：上游 `assets/GuanDan/Audio/bg.mp3` 独立导入为 `music/niuma/table_theme`；使用单独循环声道，受设置页“音乐开关/音乐音量”控制，不受音效开关干扰。
...
| 2 | 你的牌打得太好啦 | 你的牌打得太好啦 | `niuma/chat_nice_play` / `niuma-male/chat_nice_play` | 男女文本均与原声逐字一致，接入 |
...
- Male/Female 的 `feiji.mp3`、`yapai.mp3` 和不匹配快捷语均保持排除；男声包仅通过显式设置启用。
