// Regression: actual client/service modules, in-memory HTTP and platform store.
...
async function main() {
  ...
  try {
    for(const format of ['rounds','upgrade','rotating','duplicate'])for(const enabled of [false,true]){
      ...
      assert.equal(f.requests[0].body.roomSettings.counterEnabled,enabled)
      assert.equal(stored.counterEnabled,enabled)
      assert.equal(entry.roomSettings.counterEnabled,enabled)
      assert.equal(hudCounter(entry.roomSettings),enabled)
      assert.equal(hudCounter(draft),enabled,'direct configured-HUD control')
      counters.push({format,requested:enabled,submitted:enabled,stored:enabled,hudEnabled:enabled})
    }
    const changes=[
    ...
    for(const [format,choice,label,field] of changes){
      ...
      assert.notEqual(f.requests[1].body.entryAttemptId,oldId)
      assert.equal(recovered.roomSettings[field],changed[field])
      collisions.push({field,changedIntentAccepted:true})
    }
