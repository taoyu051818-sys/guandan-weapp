...
for (const score of [300, 2000, 10000]) assert.match(lobbyCatalog, new RegExp(`score: ${score}, art:`))
assert.match(lobbyCatalog, /CLASSIC_ROOM_MODES = CLASSIC_MODES/)
assert.doesNotMatch(lobbyCatalog, /team-turn|upgrade80/)
...
assert.match(lobbyPage, /dependencies\.beginMatch\(classicQueueId\(mode\.id, tier\.score\),[\s\S]*?'classic-rooms'\)/, 'each mode and stake pair must enter its own matchmaking queue')
...
assert.match(profileSection, /platformConfigured && !profile \? pendingName/)
assert.match(profileSection, /player\.loadedAt > 0 && !player\.loading \? '同步未完成' : '账号同步中'/)
...
assert.match(gatewayContracts, /CLASSIC_QUEUES\.map\(queue => queue.id\)/, 'client queue ids derive from the shared mode catalog')
assert.match(classicStakesServer, /CLASSIC_QUEUES\.map\(queue => \[queue.id, queue.stake\]\)/, 'server stake admission derives from the same catalog')
