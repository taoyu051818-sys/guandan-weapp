// Audit-only. Actual controllers/model/reducer; synthetic ports, no network or GPU.
'use strict'
...
async function replayPage() {
  ...
  assert.ok(ui.buttons.some(n=>n.text==='暂停'),'background pause leaves rendered button stale until rerender')
  click('暂停')
  assert.equal(domain.activeReplayTimeline.isPlaying,true,'visible pause button instead starts playback')
  ...
  return {backgroundPauseLabelMismatch:true,normalPauseControl:true,seekCancelsOldTimer:true,lateDetailNoRouteSteal:true}
}
async function main(){
