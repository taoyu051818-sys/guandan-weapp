...
const action = cards => {
  const resolution = core.resolvePlay(cards)
...
try {
  core.setRuleProfileByPreset('classic')

  const pairWildcard = wildcard()
  const pair = core.resolvePlay([...take('diamond', 5), pairWildcard])
  ...
  const straightFlush = core.resolvePlay(straightFlushCards)
  ...
  const tripleWithPairInfos = core.getPlayInfos(tripleWithPairCards)
  ...
  const tripleWithPair = core.resolvePlay(tripleWithPairCards)
  ...
  const rocket = core.resolvePlay(rocketCards)
  const fourBomb = core.resolvePlay(fourBombCards)
  const sixBomb = core.resolvePlay(sixBombCards)
  ...
  assert.equal(core.canPlay(rocketCards, action(sixBombCards)), true, '四王应压过六张炸弹')
  assert.equal(core.canPlay(straightFlushCards, action(fourBombCards)), true, '同花顺应压过四张炸弹')
  assert.equal(core.canPlay(fourBombCards, action(straightFlushCards)), false, '四张炸弹不能压过同花顺')
  assert.equal(core.canPlay(sixBombCards, action(straightFlushCards)), true, '六张炸弹应压过同花顺')

  const lowerPair = takeRank(4, 2)
  const higherPair = takeRank(5, 2)
  assert.equal(core.diagnosePlay(higherPair, action(lowerPair)).code, 'valid', '可压过的选择应明确返回 valid')
  assert.equal(core.diagnosePlay(lowerPair, action(higherPair)).code, 'not-high-enough', '同牌型但点数不足应明确返回 not-high-enough')
  assert.equal(core.diagnosePlay(takeRank(6, 3), action(higherPair)).code, 'type-mismatch', '不同普通牌型应明确返回 type-mismatch')
  assert.equal(core.diagnosePlay([take('diamond', 3)[0], take('club', 4)[0]], action(higherPair)).code, 'invalid-combination', '无法组成牌型时应明确返回 invalid-combination')
  assert.equal(core.diagnosePlay(higherPair, action(sixBombCards)).code, 'requires-bomb', '面对炸弹的普通牌应提示需要更大炸弹')
  assert.equal(core.diagnosePlay(fourBombCards, action(sixBombCards)).code, 'bomb-too-small', '较小炸弹应明确返回 bomb-too-small')
  assert.equal(core.diagnosePlay(sixBombCards, action(rocketCards)).code, 'rocket-unbeatable', '天王炸之后应明确返回不可压过')
  assert.equal(core.diagnosePlay([], action(higherPair)).code, 'empty', '空选择应返回 empty')
  ...
  assert.equal(core.resolvePlay(presetAceLow)?.type, core.PlayType.Straight, 'classic 应允许 A2345')
  assert.equal(core.resolvePlay(presetAceLow)?.maxValue, 5)

  core.setRuleProfileByPreset('tournament')
  assert.equal(core.resolvePlay(presetAceLow), null, 'tournament 应禁止 A2345')
  const naturalDiamondStraight = [
    ...take('diamond', 3), ...take('diamond', 4), ...take('diamond', 5),
    ...take('diamond', 6), ...take('diamond', 7),
  ]
  assert.equal(core.resolvePlay(naturalDiamondStraight)?.type, core.PlayType.Straight, 'tournament 中同花连续牌按普通顺子处理')
} finally {
  core.setRuleProfileByPreset('classic')
}
