...
| 比赛匹配等待页 | ✅ 已实现（普通场机器人补位待真机验收） | 匹配票据轮询、取消/已分配竞态恢复、等待阶段和本地已等待时长已接；quick/classic 最早真人等待满 7 秒后由平台事务补齐签名机器人席位，赛事不补；后端不返回人数/ETA 时不伪造 | `tests/match-waiting-regression.cjs`、`tests/platform-live-contract.cjs`、`server/platform/matchmaking-service.test.mjs`、`server/weapp-match-bot-ticket.smoke.mjs` | 真机验证 1–3 真人补位、7 秒边界取消、弱网重复轮询、前后台切换、自动开局、重连和入桌过期 |
| 智能理牌 | ✅ 已实现 | 显式区分普通点数堆与智能理牌：默认按实体点数归列，四张以上不自动跳左；点击理牌后才用共享规则识别并提升天王炸、炸弹、同花顺、钢板、三连对、三带二。普通单/对/三按有效点数交错，级牌在王与 A 之间；牌面不缩放，每张固定露出 40px，普通牌与大小王一致且不随叠数压缩，操作区位于上层且不避让 | `tests/hand-grouping-regression.cjs`、`tests/selection-regression.cjs` | 27 张牌、逢人配、多解冲突、炸弹强度、级牌、低端机布局和叠牌触控 |
| 手动锁牌/解锁 | ✅ 已实现 | 锁牌草稿、理牌模式和规则出牌选择三者隔离；只允许锁定当前 RuleProfile 下的合法组合。显式锁组整体进入左侧锁定区并按共享规则强度稳定排序，不会让其余牌隐式进入智能理牌；任一可见成员可选中整组出牌或进入解锁，解锁后回到当前布局规则。理牌期间修改锁后复原仍保留当前锁；权威同步使组员变化时旧锁立即释放 | `tests/table-hand-interaction-controller-regression.cjs`、`tests/hand-grouping-regression.cjs` | 己方/他方回合、与拖选并用、重连、重复 cardId、解锁后再理牌 |
| 保护组合提示 | ✅ 已实现 | shared-core 对全部合法实体候选做确定性损伤排序；锁组部分拆分采用两阶段硬保护，其余依次计算拆天王炸/炸弹、使用炸弹、拆组合/三张/对子、红桃级牌消耗和压制幅度；手牌、上一手、轮次、规则或保护组变化后从最佳候选重新开始 | `shared-core/tests/hand-hint-policy.test.ts`、`tests/table-hand-interaction-controller-regression.cjs`、`tests/local-match-controller-regression.cjs` | 锁组无替代牌提示、A 级红桃配、多个重叠可行牌型、连续提示顺序 |
| 分组高级调整与历史 | 🟡 底层能力 | `HandGrouping` 有移动、拆组、撤销/重做和恢复默认的纯状态 API；正式牌桌当前只接入锁牌/解锁和一键理牌/复原，未接移动与撤销 UI | `tests/hand-grouping-regression.cjs` | 若接 UI，需补拖动冲突、连续撤销、横竖屏和触控可达性 |
...
| 商户后台 | 🟡 迁移边界 | 服务端有 pending 申请、门店、owner/manager/cashier、日限额、积分发放幂等和自发分拦截；页面源码已移至包外 `migration/merchant/` 并独立类型检查，网关契约暂留用于迁移验证，普通玩家入口和 Cocos 运行包均不包含商户页面。仍缺审核/申诉、撤销、核销、对账、独立运营后台、完整审计和生产数据库 | `server/platform/platform.test.mjs`、`tests/platform-api-regression.cjs`、`tests/merchant-console-regression.cjs`、`tests/more-feature-retirement-regression.cjs` | 确定独立商户产品入口及鉴权边界后，再做四类角色越权、审核、幂等重试、日切、真机输入和生产事务验收 |
