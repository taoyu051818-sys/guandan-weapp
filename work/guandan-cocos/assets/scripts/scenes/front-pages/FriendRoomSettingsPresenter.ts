...
import { createFriendFormScroll, friendRoomEntryActions } from './FriendRoomFormUi'
import { showFriendRoomNumberModal } from './FriendRoomNumberModal'
import { showFriendRoomRulesModal } from './FriendRoomRulesModal'
import { renderFriendRoomModeTabs } from './FriendRoomModeTabs'
...
export class FriendRoomSettingsPresenter {
  ...
  public show (): void {
    ...
    ui.panel('FriendRoomTint', 0, 0, viewport.width, viewport.height, { fill: new Color(7, 42, 24, 78), lineWidth: 0, frame: 'square' })
    ...
    ui.panel('FriendModePanel', leftX, -8, leftWidth, panelHeight, {
      fill: new Color(12, 58, 36, 208), lineWidth: 0, frame: 'square',
    })
    ...
    renderFriendRoomModeTabs(ui, this.settingsDraft.format, leftX, leftWidth, panelHeight, topic => this.guard(generation, () => {
        this.scroll?.scrollToTop(0)
        this.selectedTab = 'rules'
        this.setSettings(changeFriendRoomFormat(this.settingsDraft, topic))
    })())

    this.compactButton(ui, '玩法规则', leftX, -panelHeight / 2 + 32, leftWidth - 18, 48, 22, generation,
      () => showFriendRoomRulesModal(viewRoot, this.dependencies.screen, this.settingsDraft.format ?? 'rounds'))
    ...
    FRIEND_ROOM_SETTINGS_TABS.forEach((tab, index) => {
      ...
      const node = ui.button('FriendSettingsTab', tab.label, x, tabWidth, 44, 22, {
        ...
        textOutlineColor: new Color(35, 65, 41), textOutlineWidth: 2, frame: 'control',
      })
    ...
    const rounds = this.selectedTab === 'rules' && this.settingsDraft.format !== 'upgrade'
    ...
    friendRoomEntryActions(ui, contentX, -panelHeight / 2 + 34, contentWidth,
      this.guard(generation, () => showFriendRoomNumberModal(viewRoot, this.dependencies.screen, roomId => this.guard(generation, () => this.dependencies.joinRoom(roomId))())), this.guard(generation, () => this.dependencies.createRoom({ ...this.settingsDraft })))
  ...
  private stepperRow (
    ...
    ui.panel('FriendSettingsRowBand', centerX, y, width, 42, { fill: new Color(8, 47, 31, 138), lineWidth: 0, frame: 'tag' })
    ...
    ui.panel('FriendStepperValueFrame', controlsCenter, y, valueWidth, 34, { fill: new Color(224, 234, 213, 245), stroke: new Color(129, 158, 117), lineWidth: 1, frame: 'tag' })
