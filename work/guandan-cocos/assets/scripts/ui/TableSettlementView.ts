    ...
    if (!this.root?.isValid || this.key !== key) {
      ...
      content.players.slice(0, 4).forEach((player, index) => {
        const card = face.panel(`SettlementRank-${index + 1}`, 0, 74 - index * 48, 700, 42, {
          fill: player.team === '我' ? new Color(52, 85, 98) : new Color(29, 65, 85), lineWidth: 0, frame: 'panel',
        })
        ...
        coastalText(face, name, -20, 0, 240, 30, 23, { parent: card, bold: true })
        coastalText(face, player.ready, 234, 0, 206, player.ready.includes('\n') ? 42 : 30, player.ready.includes('\n') ? 17 : 20, { parent: card, color: player.ready === '已准备' ? new Color(158, 232, 182) : new Color(182, 202, 207) })
      })
