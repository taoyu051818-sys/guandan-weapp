...
const DEALING_RULES: RuleSection = { title: '发牌方式', paragraphs: [
  '随机发牌：两副牌充分打乱后分发，每人27张。不洗牌（多炸）：每局重新生成成组牌堆，随机切牌并分发，使同点数牌更集中；不复用上一局的收牌顺序，也不保证任何座位必有炸弹。',
  '发牌方式不改变牌型大小、红心配和本玩法计分。复式只发一次牌，两桌对应方位保持相同手牌。大厅不洗牌匹配为随机级牌单局，好友房仍按房主设置的局数或升级目标结束。',
] }
