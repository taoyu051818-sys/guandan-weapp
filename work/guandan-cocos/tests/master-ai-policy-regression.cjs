...
const localMatch = read('assets/scripts/game/LocalMatchController.ts')
const localAITurns = read('assets/scripts/game/LocalAITurnController.ts')
const localAIEngine = read('assets/scripts/game/SynchronousLocalAIEngine.ts')
...
assert.match(sessionModel, /export const APPLICATION_AI_DIFFICULTY = 'master' as const/, 'the application AI difficulty must be a master-only literal')
assert.match(sessionModel, /difficulty: typeof APPLICATION_AI_DIFFICULTY/, 'session snapshots must not accept lower AI tiers')
...
assert.match(manager, /new LocalAITurnController\(/, 'the Cocos adapter must delegate delayed bot turns to the application controller')
...
assert.match(scene, /startMasterBotTest: \(\) => this\.tableMatch\?\.startMasterBotTest\(\)/, 'the More-menu master bot test must be wired to the match coordinator')
assert.match(tableMatchCoordinator, /public startMasterBotTest[\s\S]*frontPages\.hideAll\(\)[\s\S]*setTableVisible\(true\)[\s\S]*session\.beginLocalGame\('standard'\)[\s\S]*manager\.startRound\(\)/, 'the bot test must enter an isolated local standard table')
